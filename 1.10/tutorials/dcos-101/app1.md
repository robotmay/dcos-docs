...
* Deploy app
  * The python script has a dependency on the [redis-python](https://pypi.python.org/pypi/redis) library, which you cannot assume to be present on all agent nodes. Because of this, you should run it in the `mesosphere/dcos-101` Docker container that provides all of the dependencies. Feel free to take a look at the [DOCKERFILE](https://github.com/joerg84/dcos-101/blob/master/app1/DOCKERFILE), which was used to create the `mesosphere/dcos-101` image.
  ...
  * Add the app to Marathon using the app definition: `dcos marathon app add https://raw.githubusercontent.com/joerg84/dcos-101/master/app1/app1.json`
...
* DC/OS CLI: You have just used this option to deploy your app1. To get more information on the marathon CLI use `dcos marathon app --help`.
